...
(function(win, doc) {
    ...
    define('exec', function(require, exports, module) {
        var jsToNative = function(config) {
            ...
            // }
            // return result;
            // win.document.getElementById('demo').innerHTML = JSON.stringify(config);
            var url = config.method + '@json=' + JSON.stringify(config),
                xhr = new XMLHttpRequest();
            xhr.onreadystatechange = function() {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    var result = xhr.responseText;
                    win.document.getElementById('demo').innerHTML = JSON.stringify(result);
                    try {
                        result = JSON.parse(result);
                        if (result.code == 200) {
                            return result.result;
            ...
            win.document.getElementById('demo').innerHTML = 'loading...';
        };
        module.exports = {
